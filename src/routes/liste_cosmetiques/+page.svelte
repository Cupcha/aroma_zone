...
<body>
    ...
    <div id="container" class="bg-gray-200 pt-28">
		<product-facet>
			...
			<h1 class="font-bold text-browntext text-center text-4xl py-7 ">COSMETIQUE</h1>
			...
			{:then data}
			  <product-list class="flex text-browntext text-center">
				<div class="max-w-2xl mx-auto px-4 py-8 lg:max-w-7xl grid grid-cols-2 gap-y-10 gap-x-8 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 md:h-auto">
				  {#each data as cosmetique, i}
					<div class="bg-white shadow-lg rounded-lg">
					  <a href="/cosmetique{i+1}">
						<img src={cosmetique.picture} alt="cosmetique" class="rounded-t-lg">
					  </a>
					  <div class="p-4">
						<div class="mb-5">
						  <a href=" ">
							<h3 class="text-l text-center font-bold text-lg">{cosmetique.name}</h3>
						  </a>
						  <p class="ml-auto text-l font-medium ">{cosmetique.price}€</p>
						</div>
